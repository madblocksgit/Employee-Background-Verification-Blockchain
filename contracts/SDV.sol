...
contract SDV{
	...
	function addNewCustomer(address _userAddress, string memory _name, string memory _pAddress,string memory _cAddress, string memory _email, string memory _phone, string memory _refEmail) public notRegisteredCustomer (_userAddress)  returns (bool success) {
		users[_userAddress] = User(_name, State.Pending, _pAddress, _cAddress, _email, _phone, _refEmail, msg.sender);
		return true;
	}
	
	//Admin: Create New Org 
	function addNewOrg(address _oAddress, string memory _name, OrgType _type) public onlyOwner notRegisteredOrg(_oAddress) returns (bool success) {
		organisations[_oAddress] = Organisation(_name, _type, State.Approved);
		return true;
	}

    //Org: Verify a customer
	function verifyCustomer(address _uaddress) public registeredOrg(msg.sender) returns (bool success){
	    users[_uaddress].state = State.Approved;
	    return true;
    }
    
    //Org: Verify a document
	function verifyDocument(address _uaddress, string memory _docId) public registeredOrg(msg.sender) returns (bool success){
	    documents[_uaddress][_docId].state = State.Approved;
	    return true;
    }
    
     //Org: Reject a document
	function rejectDocument(address _uaddress, string memory _docId) public registeredOrg(msg.sender) returns (bool success){
	    documents[_uaddress][_docId].state = State.Rejected;
	    return true;
    }
    
    //Org: Send BGV Request to customer
	function sendBgvRequest(address _uAddress) public registeredOrg(msg.sender) returns (bool success) {
    ...
    function checkBgvStatus(address _uAddress) public view registeredOrg(msg.sender) returns (State) {
        
        return kycRequests[_uAddress][msg.sender].state;
    }
    
    // Org: Get User Details
	function getUserBasicDetails(address _userAddress) public view registeredOrg(msg.sender) returns (string memory, State, string memory, string memory, string memory, string memory, string memory) {
	...
	function getUserDocument(address _userAddress, string memory docId ) public view registeredOrg(msg.sender) returns (string memory, DocType , State) {
	...
	function getMyDocument(string memory docId ) public view registeredCustomer(msg.sender) returns (string memory, DocType , State) {
	        Document memory doc = documents[msg.sender][docId];
	       return (doc.name, doc.docType, doc.state);
	}
	
	
	function getMyBasicDetails() public view registeredCustomer(msg.sender) returns (string memory, State, string memory, string memory, string memory, string memory, string memory) {
		User memory user = users[msg.sender];
	    return (user.uname, user.state, user.permanentAddress, user.currentAddress, user.email, user.phone, user.refEmail);
	}
	
	 //Customer: Send BGV Request to customer
	function InitiateVerificationRequest(address _oAddress) public registeredCustomer(msg.sender) returns (bool success) {
	...
	function updateCustomer(string memory _name, string memory _pAddress,string memory _cAddress, string memory _phone, string memory _refEmail) public registeredCustomer(msg.sender) returns (bool success) {
	...
	function addDocument(string memory _docName, address _IssuedBy, DocType _doctype, string memory _docHash) public registeredCustomer(msg.sender) returns (bool success){
	...
	function removeDocument(string memory _docHash) public registeredCustomer(msg.sender) returns (bool success){
    ...
    function approveBgvRequest(address _oAddress, bool _canViewEducation, bool _canViewEmployer, bool _canViewAddress, bool _canViewReferences, bool _canViewContactDetails, bool _canViewAadhar, bool _canViewPan, bool _canViewDL) public registeredCustomer(msg.sender) returns (bool success){
    ...
    function rejectBgvRequest(address _oAddress) public registeredCustomer(msg.sender) returns (bool success){
        require(kycRequests[msg.sender][_oAddress].state != State.NotExist, "BGV Request does not exist.");
